...
import errno, zlib, random, struct, logging
...
class Common(object):
    ...
    def select_gae_ip(self, verify):
        ...
        self.GAE_IP = gae_ip
        self.GAE_SERVERS = ['%s://%s:%s/%s' % (scheme, x, gae_port, self.GAE_PATH.lstrip('/')) for x in self.GAE_HOSTS]
        self.GAE_SERVER_RAW = '%s://%s:%s/%s' % (scheme, gae_ip, gae_port, self.GAE_PATH.lstrip('/'))
    ...
    def select_fetchserver(self, url):
        ...
        if len(self.GAE_HOSTS) == 1:
            ...
            gae_host = gae_host or random_choice(self.GAE_HOSTS)
        if not common.GAE_PROXY or common.GAE_PREFER == 'https':
            gae_server = self.GAE_SERVER_RAW
        else:
            gae_server = 'http://%s/%s' % (gae_host, urlparse.urlparse(common.GAE_SERVER_RAW).path.lstrip('/'))
        return gae_server, gae_host
...
class GaeFetcher(BaseFetcher):
    ...
    def _fetch(self, url, method, headers, payload):
        errors = []
        params = gae_encode_data({'url':url, 'method':method, 'headers':headers, 'payload':payload})
        params = zlib.compress(params)
        for i in range(1, 3):
            try:
                continued, selected = 0, ''
                gae_server, gae_host = common.select_fetchserver(url)
                logging.debug('GaeFetcher fetch %r from (%r, %r)', url, gae_host, gae_server)
                ...
                # google ssl is down, set selected to trigger common.select_gae_ip(1)
...
class ConnectFetcher(BaseFetcher):
    ...
    def _direct(self):
        ...
        try:
            ...
            self.handler.send_error(502, 'Connect._direct Error')
        finally:
